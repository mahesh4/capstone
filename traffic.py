...
class Lane():
	...
	def __init__(self, led, sensor, lane_length = 50, width=3, car_length=5, interval = 10,in_flow_rate=None, out_flow_rate=None):
		self.led = led
		self.sensor = sensor
	...
	def calc_utility(self):
		queue_length = self.lane_length - self.get_distance()
		self.util_if_red = queue_length + self.in_flow_rate * self.interval 
		self.util_if_green = self.util_if_red - self.out_flow_rate * self.interval

	def return_utility(self, status):
		if status:
			return self.util_if_green
		else:
			return self.util_if_red
